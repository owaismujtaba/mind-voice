...
def plot_metric(ax, y_col, title, p_value, df_avg):
    ...
    sns.boxplot(
        ...
        hue='condition',          # required when using palette
        palette='Set3',
        width=0.5,
    ...
    # Compute means in correct order
    means = (
        df_avg.groupby('condition', sort=False)[y_col]
        .mean().reindex(category_order)
    )
    x_pos = range(len(category_order))
    ax.scatter(
        x_pos, means.values,
        marker='s', s=100, color='red', zorder=6,
        label='Mean'
    )
...
def plot_metric1(ax, y_col, title, p_value, df_avg):
    category_order = ['Visual', 'No Visual Change']
    xtick_labels = ['Visual', 'No Visual Change']
    '''
    sns.boxplot(
...
def plot_peak_mean_visual_novisual(logger):
    ...
    rest = combined_df.loc[combined_df['condition'] == category_order[1], ['mean', 'peak']].reset_index(drop=True)
    # Match subjects
