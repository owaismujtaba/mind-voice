...
class P100Analyzer:
    def __init__(self, visual_epochs, no_visual_epochs, logger, config):
        self.visual_epochs = visual_epochs
        self.no_visual_epochs = no_visual_epochs
        self.logger = logger
        self.config = config
        log_info(logger, "P100Analyzer initialized.")
        
        
    def compute_p100(self):
        self.logger.info("Computing P100 component.")        
        self.visual_evoked = self.visual_epochs.average()
        self.no_visual_evoked = self.no_visual_epochs.average()
        self.logger.info("P100 component computed.")
        
     

    def plot_p100(self, subject, session):
        self.logger.info("Plotting P100 component for subject=%s, session=%s", subject, session)
        
        channels = self.config['analysis']['p100']['ch_names']
        tmin = self.config['analysis']['p100']['tmin']
        tmax = self.config['analysis']['p100']['tmax']
    ...
    def save(self, subject=None, session=None):
        self.logger.info("Saving P100 results. sub-%s ses-%s", subject, session)
        

        results_dir = Path(self.config['analysis']['results_dir'], 'p100', 'data')
        results_dir.mkdir(parents=True, exist_ok=True)

        visual_path = results_dir / f'visual-evoked-sub-{subject}-ses-{session}.fif'
        no_visual_path = results_dir / f'no-visual-evoked-sub-{subject}-ses-{session}.fif'

        self.visual_epochs.save(visual_path, overwrite=True)
        self.no_visual_epochs.save(no_visual_path, overwrite=True)
        
        self.logger.info("P100 results saved to %s.", results_dir)
...
def run_p100_per_subject_session(config, logger, subject, session):
    ...
    visual_epocher = EpochsData(
        ...
        tmin=config['analysis']['p100']['tmin'],
        tmax=config['analysis']['p100']['tmax'],
        baseline=(config['analysis']['p100']['tmin'], 0)
    )
    visual_epochs = visual_epocher.create_epochs()  
    
    no_visual_epocher = EpochsData(
        raw = raw_eeg,
        logger = logger,
        config = config,
        event_offset=config['analysis']['p100']['no_visual_event_offset'],
        tasks=['RealWordsExperiment', 'SilentWordsExperiment'],
        event='StartFixation',
        modalities=['Pictures'], 
        tmin=config['analysis']['p100']['tmin'],
    ...
    no_visual_epochs = no_visual_epocher.create_epochs()  
    
    p100_analyzer = P100Analyzer(
        visual_epochs=visual_epochs,
        no_visual_epochs=no_visual_epochs,
        logger=logger,
        config=config
    )
    p100_analyzer.compute_p100()  
    ...
    run_best_condition_analysis(
        config, logger, 
        visual_epochs, no_visual_epochs, 
        subject, session
    )
...
def run_p100_pipeline(config, logger):
    logger.info("Starting P100 analysis pipeline.")
    ...
    for sub in subject_ids:
        ...
        for ses in session_ids:
            run_p100_per_subject_session(config, logger, sub, ses)
        
        
    logger.info("P100 analysis pipeline completed.")