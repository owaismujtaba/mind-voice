...
def plot_accuracy_per_subject(df):
    ...
    leg = ax.legend(frameon=False, fontsize=16, loc='upper left')
...
def plot_accuracy(logger):
    ...
    accuracy = load_decoding_accuracies("results/decoding/*_accuracy.csv")
    ...
    low_thr, high_thr = subj_acc['accuracy'].quantile([0.33, 0.66])

    low_perf = subj_acc[subj_acc['accuracy'] <= low_thr]
    med_perf = subj_acc[(subj_acc['accuracy'] > low_thr) & (subj_acc['accuracy'] <= high_thr)]
    high_perf = subj_acc[subj_acc['accuracy'] > high_thr]
    subj_acc.to_csv('acc.csv')
    logger.info("Low performance subjects:\n%s", low_perf)
    logger.info("Medium performance subjects:\n%s", med_perf)
    logger.info("High performance subjects:\n%s", high_perf)
