...
def plot_metrics(logger):
    ...
    metrics = load_decoding_metrics("results/decoding")
    ...
    logger.info('Metrics summary per subject:\n%s', metrics_summary.sort_values(by='f1'))
    
    subj_acc = metrics_summary.copy()
    
    low_thr, high_thr = subj_acc['f1'].quantile([0.33, 0.66])
    
    low_perf = subj_acc[subj_acc['f1'] <= low_thr]
    med_perf = subj_acc[(subj_acc['f1'] > low_thr) & (subj_acc['f1'] <= high_thr)]
    high_perf = subj_acc[subj_acc['f1'] > high_thr]
