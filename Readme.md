...
The `mind-voice` project is built primarily with Python and leverages a powerful suite of scientific computing and machine learning libraries:

*   **Python 3.8+**
*   **EEG/Neuroimaging:**
    *   `mne`: Core library for EEG/MEG data analysis.
    *   `mne-bids`: For BIDS standard compatibility.
    *   `mnelab`: Used for reading XDF files.
    *   `pyxdf`: Low-level XDF stream resolution.
    *   `pyprep`: For robust EEG preprocessing, including bad channel detection and interpolation.
*   **Machine Learning/Deep Learning:**
    *   `tensorflow`: For building and training the CNN classification models.
    *   `scikit-learn` (`imblearn`): For data splitting, evaluation metrics, and handling class imbalance (e.g., `RandomOverSampler`).
*   **Audio Processing:**
    *   `librosa`: For general audio loading and processing (e.g., pitch shifting).
    *   `praat-parselmouth`: For advanced voice manipulation, specifically formant shifting.
    *   `soundfile`: For saving audio files.
*   **Data Manipulation:**
    *   `numpy`: Fundamental package for numerical computation.
    *   `pandas`: For data structuring and analysis (e.g., loading CSVs, creating dataframes).
...
*   **Utilities:**
    ...
    *   `glob`, `os`, `re`: For file system operations and regex.
    *   `rich`: (Implicitly used by `styled_print`) for rich text and beautiful formatting in the terminal.
