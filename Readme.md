...
## Mind Voice: An Emerging Research Framework for Decoding Spanish Speech from Brainwaves

Mind Voice is a sophisticated research framework centered around a unique multimodal dataset, UGR-MINDVOICE, for analyzing electroencephalography (EEG) signals related to speech production in Iberian Spanish. Developed by researchers including Owais Mujtaba at the University of Granada, the project aims to decode both overt (spoken aloud) and covert (imagined) speech from brain activity. The framework encompasses a complete pipeline from data processing and organization to deep learning-based classification and even includes novel applications like voice anonymization.

### Core Features and Capabilities

The Mind Voice framework is designed to be a comprehensive toolkit for neuroscientific research into speech. Its main features include:

*   **Standardized Dataset Management:** The framework includes tools to organize raw EEG data into the Brain Imaging Data Structure (BIDS) format, a standard that promotes data sharing and reproducibility in neuroimaging research.
*   **Advanced EEG Preprocessing:** A robust pipeline is employed for cleaning and preparing the EEG data. This involves identifying and interpolating bad channels, filtering the signal to relevant frequency bands, and using Independent Component Analysis (ICA) to remove artifacts, such as those generated by eye movements.
*   **Event-Related Potential (ERP) Analysis:** The system has a specific module for analyzing the P100 component, an event-related potential that is important in studying visual processing. This allows researchers to analyze brain responses to visual cues in the speech experiments.
*   **Brainwave Decoding with Deep Learning:** A key feature is the use of a Convolutional Neural Network (CNN) to classify different cognitive states. The model is trained to distinguish between overt speech, covert speech, and resting states based on the processed EEG data. The framework also incorporates techniques to handle imbalanced datasets, ensuring the model is not biased towards a specific state.
*   **Voice Anonymization:** A unique component of the project is a pipeline designed to anonymize audio recordings by shifting pitch and formant frequencies. This protects the identity of the speakers who contributed to the dataset.
*   **Comprehensive Visualization:** The framework is equipped to generate a wide array of visualizations to report its findings, including plots of ERPs, model accuracy, and detailed classification performance metrics like confusion matrices.
